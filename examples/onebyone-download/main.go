...
import (
	...
	"github.com/timtide/go-titan-client/blockdownload"
)
...
func main() {
	...
	bg := blockdownload.NewBlockGetter()
	...
	for _, v := range keys {
		...
		block, err := bg.GetBlock(ctx, c)
		if err != nil {
			panic(err.Error())
		}
		fmt.Println(block.Cid(), "download success")
	}
